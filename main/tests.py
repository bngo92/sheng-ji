...
class CardTest(TestCase):
    def test_consecutive(self):
        ranks1 = Cards.fromstr("S2,S3").cards
        ranks2 = Cards.fromstr("S2,S4").cards
        ranks3 = Cards.fromstr("S2,S3,S4").cards
        ranks4 = Cards.fromstr("S14,S2,H2,J17,J18").cards
...
class PlayTest(TestCase):
    ...
    def test_init(self):
        ...
        play = CardCombinations(cards, trump_suit, trump_rank)
        ...
        cards = Cards.fromstr("S2,D2").cards
        play = CardCombinations(cards, HEARTS, TWO)
        self.assertTrue(play.suit == TRUMP)
        self.assertTrue(len(play.combinations) == 2)
        ...
        # tractor
        cards = Cards.fromstr("S2,S2,S3,S3").cards
        play = CardCombinations(cards, HEARTS, FOUR)
        self.assertTrue(play.suit == SPADES)
        self.assertTrue(len(play.combinations) == 1)
        ...
        # combination
        cards = Cards.fromstr("S2,S3").cards
        play = CardCombinations(cards, HEARTS, FOUR)
        self.assertTrue(play.suit == SPADES)
        self.assertTrue(len(play.combinations) == 2)
...
class GameTest(TestCase):
    ...
    def test_game(self):
        ...
        for player in players:
            self.assertTrue(len(Cards.fromstr(player.hand)) == game.hand_size())
        self.assertTrue(len(Cards.fromstr(game.deck)) == game.reserve_size())

        player = players[0]
        self.assertIsNone(game.pickup_reserve(player))
        self.assertTrue(len(Cards.fromstr(player.hand)) == game.hand_size() + game.reserve_size())
        self.assertTrue(len(Cards.fromstr(game.deck)) == 0)

        self.assertIsNone(game.reserve(player, player.get_hand().cards[:8]))
        self.assertTrue(len(Cards.fromstr(player.hand)) == game.hand_size())
        self.assertTrue(len(Cards.fromstr(game.kitty)) == game.reserve_size())
    ...
    def test_play(self):
        ...
        hands = [Cards.fromstr(s) for s in ("D13,D12", "D13,D2")]
        player0 = players[0]
        ...
        play = CardCombinations.decode(player0.play)
        ...
        lead_play = CardCombinations.decode(game.gameplayer_set.all()[game.lead].play)
        ...
        play = CardCombinations.decode(player1.play)
        self.assertEqual(len(play.combinations), 1)
        self.assertEqual(play.suit, DIAMONDS)
